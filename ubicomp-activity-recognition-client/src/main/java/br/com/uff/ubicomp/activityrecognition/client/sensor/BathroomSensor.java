...
public class BathroomSensor extends Thread implements NodeConnectionListener
{
    ...
    public BathroomSensor(InetSocketAddress address) 
    { 
            ...
            for (int hora=0; hora<=23; hora++)
            {
                for (int minuto=0; minuto<=59; minuto++)
                {
                    for (int idUser=1; idUser <= 22; idUser++)
                    {
                        ...
                        String mensagemSensor = energyPositionActivityRepository.findOne(idUser, horaCompleta, "BATHROOM");
