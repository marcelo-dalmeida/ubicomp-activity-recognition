...
public class ExternalAreaSensor extends Thread implements NodeConnectionListener
{
    ...
    public ExternalAreaSensor(InetSocketAddress address) 
    { 
            ...
            for (int hora=0; hora<=23; hora++)
            {
                for (int minuto=0; minuto<=59; minuto++)
                {
                    for (int idUser=1; idUser <= 22; idUser++)
                    {
                        ...
                        String mensagemSensor = energyPositionActivityRepository.findOne(idUser, horaCompleta, "EXTERNALAREA");
